<div class="w-full">
    ...
    <div class="mb-6">
        <a href="{{ route('genres.create') }}" name="create"
    ...
    <div class="mb-6 flex items-center gap-4">
        ...
        <!-- Search Box -->
        <div class="relative">
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="SEARCH FOR GENRE..."
    ...
    <div class="w-full bg-black/40 backdrop-blur-sm border border-gray-700 p-6">
        ...
        <table class="table-auto w-full border-collapse text-sm">
            ...
            <tbody class="divide-y divide-gray-800">
                @foreach ($genres_data as $gd)
                    <tr class="hover:bg-white/5 transition">
                        <td class="py-3">{{ $gd->name }}</td>
                        <td class="py-3 flex flex-row items-center justify-center">
                            <a href="{{ route('genres.edit', $gd->id) }}" name='edit{{$gd->id}}'
                                class="text-blue-400 hover:text-blue-300 transition">Edit</a>
                            <a href="{{ route('genres.delete', $gd->id) }}" name='delete{{$gd->id}}'
